...
def run_realforce_calibration(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        poses = [
            ("original", "jointangleoriginal", "Keep all five fingers naturally open and flat"),
            ("opose", "jointangleopose", "Form an O with the thumb and index finger; keep the other fingers relaxed"),
            ("fist", "jointanglefist", "Make a tight fist"),
        ]
        for pose_name, key_prefix, prompt in poses:
            ...
            _calibration_print(
                f"[calibration] {pose_name}: start collecting samples {args.calibration_sample_seconds:.1f}s, "
                "please hold the pose still..."
            )
...
def _prepare_calibration_pose(pose_name: str, prompt: str, args: argparse.Namespace) -> None:
    ...
    message = f"{title}: please assume the pose: {prompt}"
    _calibration_print("\n" + "=" * 72)
    _calibration_print(message)
    _calibration_print("=" * 72)
    if args.calibration_interactive:
        try:
            input(f"{title}: press Enter to start collection once the pose is ready and held still...")
            return
        except EOFError:
            _calibration_print(f"{title}: stdin is unavailable; switching to automatic collection after a countdown.")
    ...
    while True:
        ...
        if remaining <= 0:
            break
        _calibration_print(f"{title}: {remaining:.1f}s  until collection starts. Assume the pose now: {prompt}")
